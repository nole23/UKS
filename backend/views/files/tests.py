...
class FilesTest(TestCase):
    def setUp(self):
        ...
        p = Project.objects.create(name="test", description="test", date_create=datetime.now(), type_project=False)
        rt = Root_Tree.objects.create(name_branch="master", date_create=datetime.now(), user_create=u)
        p.root_tree.add(rt)
